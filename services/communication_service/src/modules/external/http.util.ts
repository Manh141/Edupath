...
function buildUrl(baseUrl: string, path: string, query?: InternalRequestParams['query']): string {
...
export async function internalFetch<T>(params: InternalRequestParams): Promise<T> {
  const { baseUrl, path, method = 'POST', body, query, internalSecret, timeoutMs, logger } = params;
  if (!baseUrl) {
